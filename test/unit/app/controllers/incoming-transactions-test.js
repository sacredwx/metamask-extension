...
const NON_TRACKED_CHAIN_ID = '0x1337'

function getMockNetworkController (chainId) {
  return {
...
function getMockBlockTracker () {
  return {
    addListener: sinon.stub().callsFake((_, handler) => handler('0xa')),
...
describe('IncomingTransactionsController', function () {
  ...
  describe('constructor', function () {
    it('should set up correct store, listeners and properties in the constructor', function () {
      const incomingTransactionsController = new IncomingTransactionsController({
        ...
        networkController: getMockNetworkController(NON_TRACKED_CHAIN_ID),
  ...
  describe('update events', function () {
    ...
    it('should update upon latest block when started and on supported network', async function () {
      ...
      const startBlock = NON_EMPTY_INIT_STATE.incomingTxLastFetchedBlocksByNetwork[ROPSTEN]
      ...
      assert.deepStrictEqual(
        ...
        {
          incomingTransactions: {
            ...NON_EMPTY_INIT_STATE.incomingTransactions,
            '0xfake': {
              blockNumber: '10',
              hash: '0xfake',
              metamaskNetworkId: '3',
          ...
          incomingTxLastFetchedBlocksByNetwork: {
            ...NON_EMPTY_INIT_STATE.incomingTxLastFetchedBlocksByNetwork,
            [ROPSTEN]: 11,
          },
        },
    ...
    it('should update when switching to a supported network', async function () {
      ...
      await subscription()
      incomingTransactionsController.networkController.getCurrentChainId = () => ROPSTEN_CHAIN_ID
      await subscription()
      await updateStateCalled
